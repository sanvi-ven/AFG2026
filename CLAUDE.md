...
**Read this whole file before making auth/security changes** — the auth model has real, verified identity now (as of 2026-09-01), but the migration was deliberately scoped (see "Auth model" below) and several things that would be true in a "normal" Firebase app still aren't true here (most Firestore rules stay open, the backend has almost no auth-gated routes).
...
### Auth model — real identity now exists, but only owner/employee/client login was migrated
**As of 2026-09-01, all three roles use real Firebase Auth** — this replaced the previous state (hardcoded owner password, client/employee SHA-256-in-Firestore) entirely. Read this before touching login, signup, or `firestore.rules`:

- **Identity**: a Firebase Auth account per user (email/password for all three roles — no Google Sign-In in the live app, `FirebaseService`/`google_sign_in` are unused). A `role` **custom claim** (`owner`/`employee`/`client`) is the single source of truth for what someone is — set *only* server-side, never trust a client-supplied role string.
- **Who can become what**: `POST /auth/complete-signup` (`backend/app/api/v1/routes/auth.py`) is the only place a role gets granted, given an already-verified Firebase ID token. `owner` requires the caller's email to be in `settings.owner_emails` (`OWNER_EMAILS_RAW` env var, currently just `immc17289@gmail.com`); `employee` requires a real `invite_codes` entry; `client` is unconditional. `POST /auth/claim-account` is the separate pre-auth flow for turning an owner-created "dummy" client (`ClientProfileService.createDummyClient`, no email/password yet) into a real login via a one-time claim code — it creates the Firebase user itself since no one is signed in yet at that point.
- **Frontend**: `login_page.dart`/`owner_signin_page.dart` use `FirebaseAuth.instance.signIn/createUserWithEmailAndPassword` directly; the app's real entry point is `_SessionGate` in `frontend/lib/app.dart`, which now reacts to `FirebaseAuth.instance.authStateChanges()` (reading the `role` claim off a fresh ID token, then loading the matching profile) instead of a local-storage session blob — `SessionPersistenceService` is gone, Firebase's own SDK persistence replaced it. `ClientProfileService.fetchByUid`/`EmployeeProfileService.fetchByUid` (query by a `uid` field, not the doc ID — existing docs keep their old IDs, only new field added) are how a profile gets resolved from a signed-in Firebase user.
- **Migration**: `backend/scripts/migrate_existing_accounts.py` (one-off, already run against production) created a Firebase Auth account + role claim for every pre-existing `client_signups`/`employee_signups` doc that had a real `password_hash` (dummy/unclaimed clients were left untouched — they still go through claim-account), stripped `password_hash`, and generated (but didn't send) a password-reset link per account; `backend/scripts/send_reset_emails.py` is the separate, deliberately-manual step that actually emails those links.

**What's still NOT part of this migration** — don't assume otherwise:
- `firestore.rules` is still open (`allow read, write: if true`) for almost every collection. The one exception is `client_signups`, which now checks `request.auth.token.role in ['owner','employee']` (plus self-access by `uid`/email) instead of the old "anyone" rule — that change was a forced side effect of owner/employee becoming genuinely authenticated for the first time (they'd otherwise have *lost* read access, since they no longer fall through to the old `request.auth == null` public-read branch). No other collection's rules changed. A real rules rewrite covering everything else is still a planned, not-yet-started phase — and it has a prerequisite: `messages_page.dart`, `estimates_page.dart`, `invoices_page.dart`, `appointments_page.dart`, and `reminder_check_service.dart` all call `ClientProfileService.watchAllProfiles()` unconditionally regardless of role today, which would break for client-role sessions the moment that collection's read gets restricted further.
